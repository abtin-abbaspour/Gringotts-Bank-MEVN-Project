...
export default {
      ...
      created() {
        let uri = `http://localhost:4000/accounts/find/${this.$route.params.id}`;
      ...
      methods: {
        updateAccount() {
        	if(this.account.username === this.oldUsername && this.account.password === this.oldPassword && this.account.password === confirmPassword){
